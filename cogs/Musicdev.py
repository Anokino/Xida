    ...
    async def connect(self, ctx, channel: discord.VoiceChannel = None):
        ...
        ------------
         channel: discord.VoiceChannel [Optional]
            Le salon auquel se connecter. Si aucun salon n'est spécifié, une tentative de rejoindre le canal vocal dans lequel vous vous trouvez sera faite.
        Cette commande gère également le déplacement du bot vers différents salons.
        """
        #if channel is None:
        #    try:
        channel = ctx.message.author.voice
        #    except AttributeError:
        #        await ctx.send('Aucun salon à rejoindre. Veuillez spécifier un salon valide ou joindre un canal.')
        #        raise InvalidVoiceChannel('Aucun salon à rejoindre. Veuillez spécifier un salon valide ou joindre un canal.')
                
        #await ctx.send(channel)
        
        vc = await channel.connect()
        await ctx.send(vc.channel.id)
        #vc = ctx.voice_bot
        #if vc:
        ...
        try:
            #await vc.connect(vc.channel.id)
            await vc.move_to(channel)
        except asyncio.TimeoutError:
            raise VoiceConnectionError(f'Je me déplace sur le salon : <{channel}> timed out.')
        else:
            try:
                await channel.connect()
            except asyncio.TimeoutError:
                raise VoiceConnectionError(f'Connexion au salon : <{channel}> timed out.')

        await ctx.send(f'Connecté sur: **{channel}**', delete_after=20)
